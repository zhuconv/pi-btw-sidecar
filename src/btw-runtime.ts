...
import { discoverBtwAgents, findBtwAgentByName, type BtwAgentDefinition } from "./agent-discovery";
import { buildBtwAgentListSummary, buildBtwAgentSelectionMenu } from "./agent-selection-ui";
import { loadBtwConfig, type BtwConfig, type BtwModalSize } from "./config";
...
export function buildOverlayTranscript(
  ...
  const markdownTheme = createBtwMarkdownTheme(theme);
  ...
  const renderMarkdownBlockLines = (text: string, indent = blockIndent): string[] => {
    const markdownWidth = Math.max(1, contentWidth - visibleWidth(indent));
    return renderBtwMarkdownLines(text, markdownTheme, markdownWidth);
  };
...
type MarkdownCapableTheme = ExtensionContext["ui"]["theme"] & {
  strikethrough?: (text: string) => string;
  underline?: (text: string) => string;
};

function createBtwMarkdownTheme(theme: ExtensionContext["ui"]["theme"]): MarkdownTheme {
  const markdownTheme = theme as MarkdownCapableTheme;
  return {
    ...
    strikethrough: (text) => markdownTheme.strikethrough?.(text) ?? text,
    underline: (text) => markdownTheme.underline?.(text) ?? text,
  };
}

function renderBtwMarkdownLines(text: string, markdownTheme: MarkdownTheme, width: number): string[] {
  if (text.length === 0) {
    return [""];
  }

  try {
    const rendered = new Markdown(text, 0, 0, markdownTheme).render(Math.max(1, width));
    if (rendered.length > 0) {
      return rendered.map((line) => line.trimEnd());
    }
  } catch {
    // Fall back to plain transcript lines when markdown rendering cannot parse a response.
  }
...
class BtwOverlayComponent extends Container implements Focusable {
  ...
  private getDialogHeight(): number {
    return resolveBtwModalDimensions(this.tui, this.config.modalSize).maxHeight;
  }
  ...
  override render(width: number): string[] {
    ...
    this.transcriptLines = buildOverlayTranscript(this.readTranscriptEntries(), this.theme, innerWidth, this.config);
  ...
  refresh(): void {
    ...
    this.transcriptLines = buildOverlayTranscript(entries, this.theme, 80, this.config);
...
export default function (pi: ExtensionAPI) {
  const debugLogger = createBtwDebugLogger();
  const btwIcons = resolveBtwIcons().icons;
  ...
  async function createBtwSubSession(
    ...
    const { session } = await createAgentSession({
      ...
      noTools: "all",
      tools: [],
      resourceLoader: createBtwResourceLoader(ctx, [sanitizeBtwAgentSystemPrompt(agent.systemPrompt)], createBtwSessionExtensions(settings)),
    });
  ...
  async function runBtw(
    ...
    try {
      ...
      setTranscriptFailure(transcriptState, errorMessage, btwIcons);
